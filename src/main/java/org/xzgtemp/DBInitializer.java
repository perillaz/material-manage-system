...
public class DBInitializer {
	...
	public void init() {
		...
		jdbcTemplate.update("CREATE TABLE IF NOT EXISTS Book(id BIGINT PRIMARY KEY NOT NULL AUTO_INCREMENT,title VARCHAR(50) NOT NULL,author VARCHAR(50) NOT NULL,copyamount INTEGER NOT NULL DEFAULT 0,borrowcopys INTEGER NOT NULL DEFAULT 0,allborrowtimes INTEGER NOT NULL DEFAULT 0,isbn VARCHAR(13),edition VARCHAR(30),publishtime Date,publisher VARCHAR(30),lang VARCHAR(10),briefinfo VARCHAR(1000))");
		...
		jdbcTemplate.update("CREATE TABLE IF NOT EXISTS Document(id BIGINT PRIMARY KEY AUTO_INCREMENT,title VARCHAR(30) NOT NULL,author VARCHAR(30) NOT NULL,uploaderid VARCHAR(13) NOT NULL,uploadertitle VARCHAR(30) NOT NULL,uploadtime DATE NOT NULL,filepath VARCHAR(100) NOT NULL,downloadtimes INTEGER NOT NULL,doi VARCHAR(20),literature VARCHAR(30))");
		...
		jdbcTemplate.update("CREATE TRIGGER UserNameSynchronization AFTER UPDATE ON User FOR EACH ROW IF NEW.name<>OLD.name THEN BEGIN UPDATE Book SET buyername = NEW.name WHERE buyerid = NEW.id;UPDATE Document SET uploadername = NEW.name WHERE uploader = NEW.id;END ;END IF");
